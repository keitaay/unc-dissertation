...
Figure~\ref{fig:example} shows representative examples of acquired images from all four cohorts of kidneys and of all imaging modalities. All images were taken with the transducer in the 0 degree orientation, with parametric images of VisR RE, RV, DoPIo times-intersect, and shear elasticity estimates shown overlaid on top of B-modes. Where there are appreciable differences, the VisR and DoPIo images show elastic differences in similar locations. For example, in the combined injury example (4th row), the medulla appears as a soft feature while the cortex appears to its right as a stiffer feature.
