...
Additional investigations around the forward modeling problem for DoPIo may also be considered, such as the approaches used for the FEM-based simulations in Chapter~\ref{chap:dopio} to systematically evaluate the impact of different assumptions about the material properties of the medium. This clearly involves exploring the impact of viscoelasticity on DoPIo measurements as mentioned throughout this dissertation, but acoustic properties are also pertinent to the problem as well. Initial iterations of DoPIo simulations demonstrated that acoustic attenuation distorted the shape of the ARF push beam's PSF to the point where DoPIo measurements made using different attenuation coefficients produced different elasticity estimates for the same medium (data not shown). Likewise, the increase of scatterer densities past the traditional~\cite{trahey_Properties_1988} 11 scatterers per resolution cell used in ultrasound simulations appeared to reduce the variance of DoPIo times-intersect more than the introduction or absence of white Gaussian noise in the RF data (data not shown). Future works may systematically investigate these effects to better parameterize the impacts of these and other acoustic properties on DoPIo measurements, thereby improving the fidelity of simulation-based studies.
